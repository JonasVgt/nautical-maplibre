...
function node_function()
	...
	-- Write 'seamark'
	local rank, class = GetSeamarkRank()
	if rank then WriteSeamark(class,rank) end
...
seamarkTags = Set { "beacon_cardinal", "beacon_isolated_danger", "beacon_lateral", "beacon_safe_water", "beacon_special_purpose", "buoy_cardinal", "buoy_installation", "buoy_isolated_danger", "buoy_lateral", "buoy_safe_water", "buoy_special_purpose" }
	
	--Set { "anchorage", "anchor_berth", "beacon_cardinal", "beacon_isolated_danger", "beacon_lateral", "beacon_safe_water", "beacon_special_purpose", "berth", "bridge", "bunker_station", "building", "buoy_cardinal", "buoy_installation", "buoy_isolated_danger", "buoy_lateral", "buoy_safe_water", "buoy_special_purpose", "cable_area", "cable_overhead", "cable_submarine", "causeway", "checkpoint", "coastguard_station", "communication_area", "control_point", "daymark", "distance_mark", "dredged_area", "dumping_ground", "exceptional_structure", "fairway", "ferry_route", "wall", "fog_signal", "fortified_structure", "fishing_facility", "gate", "gridiron", "harbour_basin", "harbour", "hulk", "inshore_traffic_zone", "landmark", "light", "light_major", "light_minor", "light_float", "light_vessel", "lock_basin", "marine_farm", "military_area", "mooring", "protected_area", "restricted_area", "navigation_line", "notice", "obstruction", "oil_barrier", "platform", "production_area", "pilot_boarding", "pile", "pipeline_area", "pipeline_overhead", "pipeline_submarine", "pontoon", "precautionary_area", "pylon", "radar_line", "radar_reflector", "radar_transponder", "radar_station", "calling-in_point", "radio_station", "recommended_route_centreline", "recommended_track", "recommended_traffic_lane", "rescue_station" },
seamarkClassRanks = {}
...
-- Write a way centroid to POI layer
function WriteSeamark(class,rank)
	Layer("seamarks")
	SetNameAttributes()
	--AttributeNumeric("rank", rank)
	Attribute("class", class)
	Attribute("subclass", class)
	-- layer defaults to 0
	--AttributeNumeric("layer", tonumber(Find("layer")) or 0)
end
...
function GetSeamarkRank()
	local class,rank,seamark
	class = Find("seamark:type")
	-- Can we find the tag?
	if class~="" then
		rank  = seamarkClassRanks[class] or 25
		return rank, class
	end
